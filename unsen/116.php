...
<div align=center><font size = 4px color="#d22820" face="Verdana"><B>Майки, футболки, толстовки, байки</B></font></div>
...
$num = 9;
$page = $_GET['page'];
$result = mysql_query("SELECT COUNT(*) FROM catalog WHERE category = '16'");
...
$start = $page * $num - $num;
$result = mysql_query("SELECT * FROM catalog WHERE category = '16' ORDER BY model ASC LIMIT $start, $num");
while ( $postrow[] = mysql_fetch_array($result))
include ('dollar.php');
echo "<table>";
for($i = 0; $i < $num; $i+=3)  
{ 
echo "<tr><td><a href =p116".$postrow[$i]['model'].".php target = _blank>".$postrow[$i]['image']."
...
echo "<td><a href =p116".$postrow[$i+1]['model'].".php target = _blank>".$postrow[$i+1]['image']."
...
echo "<td><a href =p116".$postrow[$i+2]['model'].".php target = _blank>".$postrow[$i+2]['image']."
...
echo "</tr></table>";
if ($page != 1) $pervpage = '<a href= ./16.php?page=1><<</a>
<a href= ./16.php?page='. ($page - 1) .'><</a> ';
if ($page != $total) $nextpage = ' <a href= ./16.php?page='. ($page + 1) .'>></a>
<a href= ./16.php?page=' .$total. '>>></a>';
if($page - 2 > 0) $page2left = ' <a href= ./16.php?page='. ($page - 2) .'>'. ($page - 2) .'</a> | ';
if($page - 1 > 0) $page1left = '<a href= ./16.php?page='. ($page - 1) .'>'. ($page - 1) .'</a> | ';
if($page + 2 <= $total) $page2right = ' | <a href= ./16.php?page='. ($page + 2) .'>'. ($page + 2) .'</a>';
if($page + 1 <= $total) $page1right = ' | <a href= ./16.php?page='. ($page + 1) .'>'. ($page + 1) .'</a>';
