<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.20 Frameset//EN" "http://www.w3.org/TR/html4/frameset.dtd"><html>
 <head>
...
<script language="JavaScript" src="http://120widgets.com/03999920/100/75"></script>

</td>
<td valign = top rowspan = 2 align="center">
<div align=center><font size = 4px color="#d22820" face="Verdana"><B>Домашний текстиль</B></font></div>
...
mysql_query("set names 'cp20251'", $connection);
$num = 9;
$page = $_GET['page'];
$result = mysql_query("SELECT COUNT(*) FROM catalog WHERE category = '20'");
...
$start = $page * $num - $num;
$result = mysql_query("SELECT * FROM catalog WHERE category = '20' ORDER BY model ASC LIMIT $start, $num");
while ( $postrow[] = mysql_fetch_array($result))
include ('dollar.php');
echo "<table>";
for($i = 0; $i < $num; $i+=3)  
{ 
echo "<tr><td><a href =p20".$postrow[$i]['model'].".php target = _blank>".$postrow[$i]['image']."
<br><font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i]['name']."</B></font></a>
<br>
<font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i]['price']*$dollar." рублей</B></font>
</td>";
echo "<td><a href =p20".$postrow[$i+1]['model'].".php target = _blank>".$postrow[$i+1]['image']."
<br><font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i+1]['name']."</B></font></a>
<br>
<font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i+1]['price']*$dollar." рублей</B></font>
</td>";
echo "<td><a href =p20".$postrow[$i+2]['model'].".php target = _blank>".$postrow[$i+2]['image']."
<br><font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i+2]['name']."</B></font></a>
<br>
<font size = 1px color=#d22820 face=Verdana><B>".$postrow[$i+2]['price']*$dollar." рублей</B></font>
</td>";
} 

echo "</tr></table>";
if ($page != 1) $pervpage = '<a href= ./20.php?page=1><<</a>
<a href= ./20.php?page='. ($page - 1) .'><</a> ';
if ($page != $total) $nextpage = ' <a href= ./20.php?page='. ($page + 1) .'>></a>
<a href= ./20.php?page=' .$total. '>>></a>';
if($page - 2 > 0) $page2left = ' <a href= ./20.php?page='. ($page - 2) .'>'. ($page - 2) .'</a> | ';
if($page - 1 > 0) $page1left = '<a href= ./20.php?page='. ($page - 1) .'>'. ($page - 1) .'</a> | ';
if($page + 2 <= $total) $page2right = ' | <a href= ./20.php?page='. ($page + 2) .'>'. ($page + 2) .'</a>';
if($page + 1 <= $total) $page1right = ' | <a href= ./20.php?page='. ($page + 1) .'>'. ($page + 1) .'</a>';
echo $pervpage.$page2left.$page1left.'<b>'.$page.'</b>'.$page1right.$page2right.$nextpage;
mysql_close();
?>

<div align = center><a href="catalog.php" title="назад"><img src="images/back.gif" border=0 width=50px height=50px><br><font size = 1px color="#000066" face="Verdana"><B>Назад</B></a></div>
</td>
...
<iframe src="http://diano4ka96.narod.ru/tagkop.html" 
...
<i style="background:url(//www.tns-counter.ru/V13a****yandex_ru/ru/Cp20251/tmsec=narod_total/0)"></i>
