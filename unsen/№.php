<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.№ Frameset//EN" "http://www.w3.org/TR/html4/frameset.dtd"><html>
 <head>
...
<script language="JavaScript" src="http://1№widgets.com/039999№/100/75"></script>

</td>
<td valign = top rowspan = 2 align="center">
<div align=center><font size = 4px color="#d22820" face="Verdana"><B>Спецодежда</B></font></div>
...
mysql_query("set names 'cp№251'", $connection);
$num = 9;
$page = $_GET['page'];
$result = mysql_query("SELECT COUNT(*) FROM catalog WHERE category = '№'");
...
$start = $page * $num - $num;
$result = mysql_query("SELECT * FROM catalog WHERE category = '№' ORDER BY model ASC LIMIT $start, $num");
while ( $postrow[] = mysql_fetch_array($result))
include ('dollar.php');
echo "<table>";
for($i = 0; $i < $num; $i+=3)  
{ 
echo "<tr><td><a href =p№".$postrow[$i]['model'].".php target = _blank>".$postrow[$i]['image']."
...
echo "<td><a href =p№".$postrow[$i+1]['model'].".php target = _blank>".$postrow[$i+1]['image']."
...
echo "<td><a href =p№".$postrow[$i+2]['model'].".php target = _blank>".$postrow[$i+2]['image']."
...
echo "</tr></table>";
if ($page != 1) $pervpage = '<a href= ./№.php?page=1><<</a>
<a href= ./№.php?page='. ($page - 1) .'><</a> ';
if ($page != $total) $nextpage = ' <a href= ./№.php?page='. ($page + 1) .'>></a>
<a href= ./№.php?page=' .$total. '>>></a>';
if($page - 2 > 0) $page2left = ' <a href= ./№.php?page='. ($page - 2) .'>'. ($page - 2) .'</a> | ';
if($page - 1 > 0) $page1left = '<a href= ./№.php?page='. ($page - 1) .'>'. ($page - 1) .'</a> | ';
if($page + 2 <= $total) $page2right = ' | <a href= ./№.php?page='. ($page + 2) .'>'. ($page + 2) .'</a>';
if($page + 1 <= $total) $page1right = ' | <a href= ./№.php?page='. ($page + 1) .'>'. ($page + 1) .'</a>';
...
<i style="background:url(//www.tns-counter.ru/V13a****yandex_ru/ru/Cp№251/tmsec=narod_total/0)"></i>
